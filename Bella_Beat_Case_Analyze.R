...
## GOAL: Analyze smart device usage data to inform Bellabeat's marketing strategy.
## AUTHOR: [Your Name/Senior Data Analyst]
## DATE: 2024-05-23
## DESCRIPTION: 
## Bellabeat is a high-tech manufacturer of health-focused products for women.
## This analysis focuses on identifying global trends in smart device usage 
## using public FitBit data to provide high-level recommendations for 
...
#    3. Gender is unknown; Bellabeat specifically targets women, but we 
...
# Note: We will focus on the primary files that address the business task.

# Update the file paths according to your local folder structure
...
# We use clean_names() to ensure all column names are consistent: 
...
# For Daily Sleep (Format: 4/12/2016 12:00:00 AM)
daily_sleep <- daily_sleep %>%
  mutate(sleep_day = mdy_hms(sleep_day))

# For Hourly Data (Format: 4/12/2016 12:00:00 AM)
hourly_steps <- hourly_steps %>%
...
# Final participant count after cleaning
